...
The simulations presented in these notes can be found at \url{https://sylvainlc.github.io/}. Most elementary numerical solutions are based on scikit-learn, \url{https://scikit-learn.org/stable/supervised\_learning.html} provides many helpful comments and advices.
...
$$
\begin{itemize}
\item In a classification setting using the 0-1 loss, the risk function is $\mathsf{R}(f) = \bP(Y\neq f(X))$.
...
is such that
\[
h_{\star} = \underset{h:\xset\to\{-1,1\}}{\argmin}L_{\mathrm{miss}}(h)\eqsp.
\]
\end{lemma}
\end{shaded}
\begin{proof}
...
%\bE\left[Y\middle|X\right] = \bP\left(Y=1\middle|X\right) - \bP\left(Y=-1\middle|X\right) = 2 \bP\left(Y=1\middle|X\right) - 1 \eqsp.
%\]
The Bayes classifier is the optimal choice to minimize the probability of misclassification $L_{\mathrm{miss}}$. However, as the conditional distribution of $Y$ given $X$ is usually unknown, it cannot be computed explicitly. Supervised classification aims at designing an approximate classifier $\widehat h_n$ using independent observations $(X_i,Y_i)_{1\leqslant i\leqslant n}$ with the same distribution as $(X,Y)$ so that the error $L_{\mathrm{miss}}(\widehat h_n)-L_{\mathrm{miss}}(h_{\star})$ may be controlled.
...
\begin{equation}
\label{eq:prob:lda}
\mathbb{P}\left(Y=1\middle | X\right) = \sigma\left( X^T\omega+ b\right)\eqsp,
\end{equation}
where
$$
\omega =  \Sigma^{-1}\left(\mu_{1} - \mu_{-1}\right)\,,
b = \log(\pi_1/\pi_{-1}) +  \frac{1}{2}\left(\mu_{1}+\mu_{-1}\right)'\Sigma^{-1}\left(\mu_{-1}-\mu_{1}\right)\eqsp.
$$
Since
\[
...
& \Leftrightarrow \pi_1 \exp\left\{-\frac{1}{2}\left(x-\mu_1\right)^T\Sigma^{-1}\left(x-\mu_1\right)\right\} > \pi_{-1} \exp\left\{-\frac{1}{2}\left(x-\mu_{-1}\right)^T\Sigma^{-1}\left(x-\mu_{-1}\right)\right\}\eqsp,\\
& \Leftrightarrow \log\left(\frac{\pi_{1}}{\pi_{-1}}\right) > -\frac{1}{2}\left(x-\mu_{-1}\right)^T\Sigma^{-1}\left(x-\mu_{-1}\right) + \frac{1}{2}\left(x-\mu_1\right)^T\Sigma^{-1}\left(x-\mu_1\right)\eqsp,\\
& \Leftrightarrow \log\left(\frac{\pi_{1}}{\pi_{-1}}\right) > x^T \Sigma^{-1}\left(\mu_{-1} - \mu_{1}\right) + \frac{1}{2}\left(\mu_{1}+\mu_{-1}\right)^T\Sigma^{-1}\left(\mu_{1}-\mu_{-1}\right)\eqsp.
\end{align*}
...
&= - \frac{nd}{2} \log(2\pi)+\sum_{i=1}^n\sum_{k\in\{-1,1\}}\1_{Y_i=k}\left(\log \pi_{k} -\frac{\log \det \Sigma}{2} - \frac{1}{2}\left(X_i - \mu_{k}\right)^T\Sigma^{-1}\left(X_i - \mu_{k}\right)\right) \eqsp,\\
...
& \hspace{1cm}-  \frac{1}{2}\sum_{i=1}^n\1_{Y_i=1}\left(X_i - \mu_{1}\right)^T\Sigma^{-1}\left(X_i - \mu_{1}\right) -  \frac{1}{2}\sum_{i=1}^n\1_{Y_i=-1}\left(X_i - \mu_{-1}\right)^T\Sigma^{-1}\left(X_i - \mu_{-1}\right)\eqsp.
\end{align*}
...
\frac{\partial \log \bP_{\theta}\left(X_{1:n},Y_{1:n}\right)}{\partial \Sigma^{-1}} &= \frac{n}{2}\Sigma -  \frac{1}{2}\sum_{i=1}^n\1_{Y_i=1}\left(X_i - \mu_{1}\right)\left(X_i - \mu_{1}\right)^T -  \frac{1}{2}\sum_{i=1}^n\1_{Y_i=-1}\left(X_i - \mu_{-1}\right)\left(X_i - \mu_{-1}\right)^T\eqsp.
\end{align*}
...
\widehat\Sigma^n &= \frac{1}{n}\sum_{i=1}^n \left(X_i - \widehat \mu_{Y_i}^n\right)\left(X_i - \widehat \mu_{Y_i}^n\right)^T\eqsp.
\end{align*}
...
%\[
%\widehat L^n_{\mathrm{miss}}(h) = \frac{1}{n}\sum_{i=1}^n \1_{Y_i \neq h(X_i)}\eqsp,
%\]
...
%\widehat h^n_{\calH} \in \underset{h\in\calH}{\argmin}\;\widehat L^n_{\mathrm{miss}}(h)\eqsp.
%\end{equation}
%In this context several practical and theoretical challenges arise from the minimization of the empirical classification risk. The choice of $\calH$ is pivotal in designing an efficient classification procedure. Note that choosing $\calH$ as all possible classifiers is meaningless, in this case, $\widehat h^n_{\calH}$ is such that $\widehat h^n_{\calH}(X_i) = Y_i$ for all $1 \leqslant i\leqslant n$ and $\widehat h^n_{\calH}(x)$ is any element of $\{-1,1\}$ for all $x\notin\{X_1,\ldots,X_n\}$. Although $\widehat L^n_{\mathrm{miss}}(h^n_{\calH}) = 0$,  is likely to be  a poor approximation of $L_{\mathrm{miss}}(h^n_{\calH})$. To understand this, the excess misclassification risk may be decomposed as follows
%\[
%L_{\mathrm{miss}}\left(\widehat h^n_{\calH}\right) - L_{\mathrm{miss}}\left(h_{\star}\right) = L_{\mathrm{miss}}\left(\widehat h^n_{\calH}\right) - \underset{h\in\calH}{\min}\; L_{\mathrm{miss}}\left(h\right)  + \underset{h\in\calH}{\min}\; L_{\mathrm{miss}}\left(h\right)  -  L_{\mathrm{miss}}\left(h_{\star}\right) \geqslant 0\eqsp.
%\]
%The first term of the decomposition $L_{\mathrm{miss}}(\widehat h^n_{\calH}) - \min_{h\in\calH}\; L_{\mathrm{miss}}\left(h\right)$ is a \textbf{stochastic error} which is likely to grow when the size of $\calH$ grows while $\min_{h\in\calH}\; L_{\mathrm{miss}}\left(h\right)  -  L_{\mathrm{miss}}\left(h_{\star}\right)$ is \textbf{deterministic} and likely to decrease as the size of $\calH$ grows.
...
%\begin{equation}
%\label{eq:stochError}
%L_{\mathrm{miss}}(\widehat h^n_{\calH}) - \underset{h\in\calH}{\min}\; L_{\mathrm{miss}}\left(h\right) \leqslant 2 \underset{h\in\calH}{\sup}\left|\widehat L^n_{\mathrm{miss}}(h) - L_{\mathrm{miss}}\left(h\right)\right| \eqsp.
%\end{equation}
%\end{lemma}
%\end{shaded}
%\begin{proof}
...
%\begin{align*}
%L_{\mathrm{miss}}(\widehat h^n_{\calH}) - \underset{h\in\calH}{\min}\; L_{\mathrm{miss}}\left(h\right) &=  L_{\mathrm{miss}}(\widehat h^n_{\calH})  - \widehat L^n_{\mathrm{miss}}(\widehat h^n_{\calH}) + \widehat L^n_{\mathrm{miss}}(\widehat h^n_{\calH}) - \underset{h\in\calH}{\min}\; L_{\mathrm{miss}}\left(h\right)\eqsp,\\
%&\leqslant L_{\mathrm{miss}}(\widehat h^n_{\calH})  - \widehat L^n_{\mathrm{miss}}(\widehat h^n_{\calH}) + \widehat L^n_{\mathrm{miss}}(h) - \underset{h\in\calH}{\min}\; L_{\mathrm{miss}}\left(h\right)\eqsp.
%\end{align*}
%For all $\varepsilon>0$ there exists $h_{\varepsilon}\in\calH$ such that  $L_{\mathrm{miss}}(h_{\varepsilon})< \min_{h\in\calH}\; L_{\mathrm{miss}}\left(h\right) + \varepsilon$ so that
%\begin{align*}
%L_{\mathrm{miss}}(\widehat h^n_{\calH}) - \underset{h\in\calH}{\min}\; L_{\mathrm{miss}}\left(h\right) &\leqslant L_{\mathrm{miss}}(\widehat h^n_{\calH})  - \widehat L^n_{\mathrm{miss}}(\widehat h^n_{\calH}) + \widehat L^n_{\mathrm{miss}}(h_{\varepsilon}) - L_{\mathrm{miss}}(h_{\varepsilon}) + \varepsilon\eqsp,\\
%&\leqslant   2 \underset{h\in\calH}{\sup}\left|\widehat L^n_{\mathrm{miss}}(h) - L_{\mathrm{miss}}\left(h\right)\right| + \varepsilon\eqsp,
%\end{align*}
%which concludes the proof.
%\end{proof}
%
...
%\[
%\bP\left(L_{\mathrm{miss}}(\widehat h^n_{\calH}) \leqslant \underset{1\leqslant j \leqslant M}{\min}L_{\mathrm{miss}}(h_j) + \sqrt{\frac{2}{n}\log\left(\frac{2M}{\delta}\right)}\right)\geqslant 1-\delta\eqsp.
%\]
%\end{proposition}
%\end{shaded}
%\begin{proof}
...
%\[
%\bP\left(L_{\mathrm{miss}}(\widehat h^n_{\calH}) > \underset{1\leqslant j \leqslant M}{\min}L_{\mathrm{miss}}(h_j) + u\right) \leqslant \bP\left( \underset{h\in\calH}{\sup}\left|\widehat L^n_{\mathrm{miss}}(h) - L_{\mathrm{miss}}\left(h\right)\right|  > \frac{u}{2}\right) \leqslant \sum_{j=1}^M  \bP\left(\left|\widehat L^n_{\mathrm{miss}}(h_j) - L_{\mathrm{miss}}\left(h_j\right)\right|  > \frac{u}{2}\right)\eqsp.
%\]
%By Hoeffding's inequality, see Theorem~\ref{th:hoeffding},
%\[
%\bP\left(L_{\mathrm{miss}}(\widehat h^n_{\calH}) > \underset{1\leqslant j \leqslant M}{\min}L_{\mathrm{miss}}(h_j) + u\right) \leqslant 2M\rme^{-nu^2/2}\eqsp,
%\]
...
%\[
%\bE\left[L_{\mathrm{miss}}(\widehat h^n_{\calH})\right] \leqslant \underset{1\leqslant j \leqslant M}{\min}L_{\mathrm{miss}}(h_j) + \sqrt{\frac{2\log\left(2M\right)}{n}}\eqsp.
%\]
%\end{proposition}
%\end{shaded}
%
...
%$$
%\bE\left[L_{\mathrm{miss}}(\widehat h^n_{\calH})\right]  - \underset{1\leqslant j \leqslant M}{\min}L_{\mathrm{miss}}(h_j) \leqslant  2 \bE\left[\underset{h\in\calH}{\sup}\left|\widehat L^n_{\mathrm{miss}}(h) - L_{\mathrm{miss}}\left(h\right)\right|  \right]=\frac{2 }{n} \bE\left[ \underset{1\leqslant j\leqslant M}{\max}\left\{n\left|\widehat L^n_{\mathrm{miss}}(h_j) - L_{\mathrm{miss}}\left(h_j\right)\right| \right\}\right]\eqsp.
%$$
%Note that
%\[
%n\left\{\widehat L^n_{\mathrm{miss}}(h_j) -  L_{\mathrm{miss}}\left(h_j\right)\right\} = \sum_{i=1}^n \left\{\1_{Y_i \neq h_j(X_i)}  -  L_{\mathrm{miss}}\left(h_j\right)\right\}\eqsp,
%\]
%where the random variables $(\1_{Y_i \neq h_j(X_i)})_{1\leqslant i \leqslant n}$ are independent Bernoulli random variables with mean $L_{\mathrm{miss}}(h_j)$. By Lemma~\ref{lem:bernoulli:bound}, for all $t>0$,
%\[
%\bE\left[\mathrm{exp}\left\{t\sum_{i=1}^n \left\{\1_{Y_i \neq h_j(X_i)}  -  L_{\mathrm{miss}}\left(h_j\right)\right\}\right\}\right] = \prod_{i=1}^n \bE\left[\mathrm{exp}\left\{t\left(\1_{Y_i \neq h_j(X_i)}  -  L_{\mathrm{miss}}\left(h_j\right)\right)\right\}\right] \leqslant \rme^{nt^2/8}
%\]
%and similarly
%\[
%\bE\left[\mathrm{exp}\left\{-t\sum_{i=1}^n \left\{\1_{Y_i \neq h_j(X_i)}  -  L_{\mathrm{miss}}\left(h_j\right)\right\}\right\}\right] \leqslant \rme^{nt^2/8}\eqsp.
%\]
%Then, for all $t>0$, by Jensen's inequality,
%\begin{align*}
%\mathrm{exp}\left\{t \bE\left[ \underset{1\leqslant j\leqslant M}{\max}\left\{n\left|\widehat L^n_{\mathrm{miss}}(h_j) - L_{\mathrm{miss}}\left(h_j\right)\right| \right\}\right]\right\} &\leqslant \bE\left[\mathrm{exp}\left\{t\underset{1\leqslant j\leqslant M}{\max}\left\{n\left|\widehat L^n_{\mathrm{miss}}(h_j) - L_{\mathrm{miss}}\left(h_j\right)\right|\right\}\right\}\right] \eqsp,\\
...
%which yields
%\[
% \bE\left[ \underset{1\leqslant j\leqslant M}{\max}\left\{n\left|\widehat L^n_{\mathrm{miss}}(h_j) - L_{\mathrm{miss}}\left(h_j\right)\right| \right\}\right] \leqslant \frac{\log (2M)}{t} + \frac{nt}{8}\eqsp.
%\]
%Choosing $t= \sqrt{8\log (2M)/n}$,
%\[
% \bE\left[ \underset{1\leqslant j\leqslant M}{\max}\left\{n\left|\widehat L^n_{\mathrm{miss}}(h_j) - L_{\mathrm{miss}}\left(h_j\right)\right|\right\}\right] \leqslant \sqrt{n\log(2M)/2}\eqsp,
%\]
%which concludes the proof.
%\end{proof}
%
